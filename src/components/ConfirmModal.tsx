...
import { ShieldAlert, AlertTriangle, X, AlertCircle } from "lucide-react";
        ...
        <div className="p-6">
          <div className="flex items-start gap-3 mb-5">
            <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ring-1"
              style={{ backgroundColor: withAlpha(accentColor, 0.1), boxShadow: `inset 0 0 0 1px ${withAlpha(accentColor, 0.15)}` }}>
              <HeaderIcon className="w-5 h-5" style={{ color: accentColor }} />
            </div>
            <div className="flex-1 min-w-0 pt-0.5">
              <h3 id="confirm-modal-title" className="text-foreground font-semibold text-[15px]">{config.title}</h3>
              <p id="confirm-modal-body" className="text-xs text-muted-foreground mt-1.5 leading-relaxed">{config.body}</p>
            </div>
          ...
          {/* 되돌릴 수 없는 작업임을 가장 먼저 인지시키기 위해 대상 정보보다 위에 배치 */}
          <div className="mb-4 flex items-start gap-2.5 px-3.5 py-2.5 rounded-lg border text-xs font-medium leading-relaxed"
            style={{ backgroundColor: withAlpha(RED, 0.05), borderColor: withAlpha(RED, 0.2), color: RED }}>
          ...
          {config.summaryItems && config.summaryItems.length > 0 && (
            <div className="mb-4 grid grid-cols-1 sm:grid-cols-3 gap-2">
              {config.summaryItems.map((s) => (
                <div key={s.label} className="rounded-lg border border-border bg-muted/20 px-3 py-2.5 flex flex-col gap-0.5">
          ...
          {bi && dmg && (
            <div className="mb-4 rounded-lg border border-border bg-muted/30 overflow-hidden">
              <div className="px-3.5 py-2 border-b border-border bg-muted/50 flex items-center justify-between">
              ...
              <div className="px-3.5 py-3 flex flex-col gap-2.5">
                <div>
                  <p className="text-[13px] font-semibold text-foreground">{bi.title}</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">{bi.author} · {bi.branch}</p>
                </div>
          ...
          {config.detail && (
            <div className="mb-5 px-3.5 py-2.5 rounded-lg border text-xs text-muted-foreground leading-relaxed"
              style={{ backgroundColor: withAlpha(accentColor, 0.04), borderColor: withAlpha(accentColor, 0.19) }}>
